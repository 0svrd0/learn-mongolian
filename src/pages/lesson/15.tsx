...
const Lesson15 = () => {
  ...
  const playAudio = () => {
    ...
    const word = LESSON15_WORDS[currentWordIndex].mongolian;
    const audio = new Audio(`/audio/lesson15/${word}.mp3`);
  ...
  const nextWord = () => {
    setCurrentWordIndex((prev) => (prev + 1) % LESSON15_WORDS.length);
  };

  const previousWord = () => {
    setCurrentWordIndex((prev) => 
      (prev - 1 + LESSON15_WORDS.length) % LESSON15_WORDS.length
    );
  };
    ...
    <div className="min-h-screen bg-mongol-cream p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="flex justify-between">
          ...
          <Button
            onClick={() => navigate("/lesson/15/quiz")}
