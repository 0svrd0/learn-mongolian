...
const Lesson5 = () => {
  ...
  const playAudio = () => {
    const currentWord = LESSON5_WORDS[currentWordIndex];
  ...
  const nextWord = () => {
    setCurrentWordIndex((prev) => (prev + 1) % LESSON5_WORDS.length);
  };

  const previousWord = () => {
    setCurrentWordIndex((prev) => 
      (prev - 1 + LESSON5_WORDS.length) % LESSON5_WORDS.length
    );
  };
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mt-8">
          <p className="text-sm text-gray-500 text-center">
            Progress: {currentWordIndex + 1} / {LESSON5_WORDS.length}
          </p>
        </div>
      </div>
    </div>
