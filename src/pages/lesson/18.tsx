...
const Lesson18 = () => {
  ...
  const playAudio = () => {
    ...
    const word = LESSON18_WORDS[currentWordIndex].mongolian;
    const audio = new Audio(`/audio/lesson18/${word}.mp3`);
  ...
  const nextWord = () => {
    setCurrentWordIndex((prev) => (prev + 1) % LESSON18_WORDS.length);
  };

  const previousWord = () => {
    setCurrentWordIndex((prev) => 
      (prev - 1 + LESSON18_WORDS.length) % LESSON18_WORDS.length
    );
  };

  const currentWord = LESSON18_WORDS[currentWordIndex];
    ...
    <div className="min-h-screen bg-mongol-cream p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <Card className="mb-8">
          <CardContent className="p-6">
            ...
            <p className="text-gray-600 mb-6">
              Learn essential Mongolian travel vocabulary
            </p>
        ...
        <div className="flex justify-between">
          ...
          <Button
            onClick={() => navigate("/lesson/18/quiz")}
        ...
        <div className="mt-8">
          <p className="text-sm text-gray-500 text-center">
            Progress: {currentWordIndex + 1} / {LESSON18_WORDS.length}
          </p>
        </div>
      </div>
    </div>
